defmodule Tide.Builder.Producer do
  ...
  def handle_info(%HTTPoison.AsyncChunk{chunk: chunk}, %{chunks: chunks, pending_demand: demand} = state) do
    {events, new_chunks} = Enum.split([Poison.decode!(chunk) | chunks], demand)
    Logger.info("Received chunk #{chunk}")

    {:noreply, events, %{state | status: :processing, chunks: new_chunks, pending_demand: demand - length(events)}}
  end
