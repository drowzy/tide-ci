...
let make = (~router, _children) => {
  let renderProjectEntry = (repo : Repo.t, ~handleClick, ~linkTo) =>
    <div className="project-list">
      <div className="tile tile-centered">
        ...
        <div className="tile-action">
          <button className="btn btn-sm" style=(ReactDOMRe.Style.make(~marginRight=".2rem", ())) onClick={linkTo(repo.id)}>(ReasonReact.stringToElement("Go to"))</button>
  ...
  let linkToProject = (id, event) => {
    ReactEventRe.Mouse.preventDefault(event);
    DirectorRe.setRoute(router, "/projects/" ++ id);
  };

  {
    ...
    render: ({ state }) => {
      switch state.repos {
        ...
        | repos => repos |> Array.map(repo => renderProjectEntry(repo, ~handleClick=(_event) => Repo.build_repo(repo.id, ignore), ~linkTo=linkToProject)) |> ReasonReact.arrayToElement
      };
    }
  }
};
